...
class Exploration_data:
    ...
    def get_count_missing_vals_in_1column(self, col_name):  #df dataframe && col_name : name of column
        """
        Count the missing values inside one column
        """
        missing_df = self.get_count_of_missing_values()
        full_columns = self.df.shape[0] - missing_df.loc[col_name][0]
        return full_columns


    def visualize_feature_types(self):
        """
        Visualize a plot bar with the number of features by type
        """
    ...
    def visualize_type_local(self):
        """
        Visualize a plot bar with the count of each type of "type local"
        """
